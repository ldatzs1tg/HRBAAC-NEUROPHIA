...
CONFIG.update(
    {
        ...
        "TIER_BLEND": {
            # ── A-Tier overrides (Dense uses WEEK_WEIGHTS directly) ──
            # A-tier Intermittent
            (0, 1): {"w14": {"A": 1.0}, "w58": {"A": 0.70, "D": 0.30}},
            # A-tier Sparse
            (0, 2): {"w14": {"A": 0.66, "D": 0.34}, "w58": {"A": 0.75, "D": 0.25}},

            # ── B-Tier overrides ──
            # B-tier Dense doesn't exist in the optimization output, defaulting to A-tier dense equivalent
            (1, 0): {"w14": {"A": 0.96, "E": 0.04}, "w58": {"A": 0.30, "E": 0.50, "D": 0.20}},
            # B-tier Intermittent
            (1, 1): {"w14": {"A": 1.0}, "w58": {"A": 0.65, "D": 0.35}},
            # B-tier Sparse
            (1, 2): {"w14": {"A": 1.0}, "w58": {"A": 0.70, "D": 0.30}},
            
            # ── C-Tier overrides ──
            # C-tier Intermittent
            (2, 1): {"w14": {"A": 0.90, "D": 0.10}, "w58": {"A": 0.70, "D": 0.30}},
            # C-tier Sparse
            (2, 2): {"w14": {"A": 0.90, "D": 0.10}, "w58": {"A": 0.75, "D": 0.25}},
            
            # ── D-Tier overrides ──
            # D-tier Intermittent
            (3, 1): {"w14": {"A": 0.63, "D": 0.37}, "w58": {"A": 0.70, "D": 0.30}},
            # D-tier Sparse
            (3, 2): {"w14": {"A": 0.90, "D": 0.10}, "w58": {"A": 0.60, "D": 0.40}},
        },
    }
)
...
def _blend_ensemble(
    ...
    for day in range(1, horizon + 1):
        ...
        for i in range(n_skus):
            ...
            if tier == 0 and density == 0:  # A-tier Dense: use week weights
                raw_w = dict(CONFIG["WEEK_WEIGHTS"].get(week_idx, {"A": 1.0}))
                # Zero out E if no ETS
                if not has_ets:
                    raw_w.pop("E", None)
            else:
                # Lower tiers (and A-tier Intermittent/Sparse): use TIER_BLEND
